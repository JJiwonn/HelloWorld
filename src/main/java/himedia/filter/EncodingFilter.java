...
public class EncodingFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		logger.info("[EncondingFilter] init");
	...
	public void doFilter(ServletRequest req, 
		...
		// 응답 정보로부터 출력을 위한 객체 얻어오기
				PrintWriter out = resp.getWriter();
				out.println("<h6>Encoding Filter 작동 전</h6>");

		// 실제 필터가 처리하는 내용
		logger.info("[EncondingFilter] doFilter");
		...
		chain.doFilter(req,resp); 	// 요청 처리 종료 후 다음 필터로 전달(핵심)
	...
	@Override
	public void destroy() {
		logger.info("[EncondingFilter] destroy");
